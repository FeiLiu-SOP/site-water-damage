/**
 * Pre-build gate: water-damage SERP titles ≤55 chars (full words only).
 * Run: npm run check:water-damage-titles
 */
...
const contentDir = path.join(__dirname, "..", "src", "content", "water-damage");
...
const libUrl = pathToFileURL(path.join(__dirname, "..", "src", "lib", "water-damage-serp-title.ts")).href;
const { buildWaterPageTitle, WATER_TITLE_MAX } = await import(libUrl);
...
for (const file of files) {
  ...
  const pick = stableHash(`water-damage|${slug}|bait2`) % 3;
  try {
    const title = buildWaterPageTitle({
      city,
      zLabel,
      zLabelExact,
      highValue,
    ...
    if (title.length > WATER_TITLE_MAX) {
      console.warn(`[water-title] Title Truncated: ${title.length}>${WATER_TITLE_MAX} slug=${slug}`);
      failures++;
    }
  } catch (err) {
...
if (failures > 0) {
  console.error(`[check-water-damage-serp-titles] ${failures} page(s) failed SERP title gate.`);
  process.exit(1);
}
console.log(`[check-water-damage-serp-titles] OK — ${files.length} files scanned, all titles ≤${WATER_TITLE_MAX} chars.`);
